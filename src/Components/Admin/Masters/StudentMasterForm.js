...
const StudentMasterForm = (props) => {
    ...
    const myInitialValues           = { enrollno: '', name: '' ,instId: '', programId:'', semester:'',mobile:'',email:'',password:''};
    ...
    return (
        ...
        onSubmit= {async (values,actions) => 
        {
            ...
            actions.resetForm({
            values: {
                        enrollno: '', name: '', instId: '', programId:'', semester:'',mobile:'',email:'',password:''
                    },
            });
        }}
...
async function saveStudent(values,setLoading,setShow,setMsg,setMyMsg,setMyList,myList)
{
    ...
    await API.post('/user',{'type':'student','username':enrollno,'name':name,'instId':instId,'programId':programId,'semester':semester,'mobile':mobile,'email':email,'password':password})
    .then((res) => 
    {
        if(res.data.status === 'success')
        {
